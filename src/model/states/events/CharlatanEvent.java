...
public class CharlatanEvent extends DailyEventState {
    ...
    protected void doEvent(Model model) {
        ...
        portraitSay(model, "... yes, it's true, this elixir will cure your stomach ailments sir. But not only that! " +
                "Have a problem with warts madam? The elixir will remove them for you!");
        portraitSay(model, "In fact, my famous draft will cure just about anything. And it tastes good too!");
        portraitSay(model, "Only available here and now! For the incredible price of just 20 gold pieces, you may " +
        ...
        portraitSay(model, "I guarantee it ladies and " +
        ...
        if (model.getParty().getGold() < 20) {
            ...
            if (yesNoInput()) {
                ...
                portraitSay(model, "A bargain, I guarantee.");
                ...
                model.getParty().partyMemberSay(model, model.getParty().getLeader(), "This is cherry juice...");
                model.getParty().randomPartyMemberSay(model, List.of("Expensive cherry juice."));
            } else {
                model.getParty().partyMemberSay(model, model.getParty().getLeader(), "I think we will pass on this one.");
            }
        }
    }
    ...
    private void charlatanFoundOut(Model model, GameCharacter gc, SkillCheckResult result) {
        ...
        portraitSay(model,heOrSheCap(gc.getGender()) + " is right. It is juice!");
        ...
        boolean lockPicked = model.getParty().doSoloSkillCheck(model, this, Skill.Security, 8);
        if (lockPicked) {
